...
- **一致性**指的是数据库总是从一个一致性的状态转换到另外一个一致性的状态。比如A转账给B100块钱，假设A只有90块，事务不能成功。数据的一致性，转账前和转账后，从前面数据状态变为后面数据状态，不能对不上数、破坏约束。
  一致性是事务要达成的目的，通过原子性、隔离性、持久性来保证一致性。
- **隔离性**：定义事务访问相同资源时，资源是否可见。
  有事务使用资源，不让其他事务使用，直到该事务将数据修改后或提交后，其他事务才能读取。
- **持久性**指的是一旦事务提交，所做的修改就会永久保存到数据库中。

隔离性有4个隔离级别，分别是：

- **读未提交**(read uncommit)，事务未提交前对其它事务也是可见的，可能会脏读。
- **读已提交**(read commit)（oracle默认）事务只能读取已提交的事务，它解决了脏读的问题。
- **可重复复读**(repeatable read) （mysql的默认），只会读取已经提交的事务，读取数据之后，后面再读取直接用第一次读的值，读多次都一样，但是有可能产生幻读。
- **串行**(serializable)，一般是不会使用的，会给每一行读取的数据加锁，会导致大量超时和锁竞争的问题。



- **脏读**(Drity Read)，可以读取到未提交的数据。某个事务已更新一份数据，另一个事务在此时读取了同一份数据，由于某些原因，前一个RollBack了操作，则后一个事务所读取的数据就会是不正确的。
- **不可重复读**(Non-repeatable read)，在一个事务的两次查询之中数据不一致，这可能是两次查询过程中间插入了更新操作。
- **幻读**(Phantom Read)，在一个事务的两次查询中数据条数不一致，比如第二次查询之前插入了新的几列数据。
