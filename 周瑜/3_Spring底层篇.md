...
- AOP面向切面。
- 系统是由许多不同的组件所组成的，每一个组件各负责一块特定功能。除了实现自身核心功能之外，这些组件还经常承担着额外的职责。例如**日志、事务管理和安全**这样的非核心服务经常融入到自身具有核心业务逻辑的组件中去。 这些系统服务经常被称为**横切关注点**，因为它们会跨越系统的多个组件。
...
AOP可以对某个对象或某些对象的功能进行增强，比如对象中的方法进行增强，可以在执行某个方法前后额外的做一些事情。比如一个接口里面没有写打印日志逻辑，但是自动拥有打印日志功能。

## 谈谈你对IOC的理解IOC就是控制反转，指创建对象的控制权转移给Spring来进行管理。简单来说，就是应用不用去new对象了，而全部交由Spring自动生产。
